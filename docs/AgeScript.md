...
- Любой вызов функции должен быть либо присвоен переменной, либо помещён в условие `if`, иначе движок его проигнорирует
...
Операторы `*`, `/` - имеют более высокий приоритет.  
Скобки позволяют поднять приоритет
```c
// Приоритет операторов
a = 1 + 2 * 3; // a is "7"
...
## Оператор goto
Поддерживается в соответствии с [C99](https://en.cppreference.com/w/c/language/goto.html)
```c
// Пример использования для имитации цикла while
i = 1;
start:
...
Фигурные скобки у тела оператора необязательные. Наличие `else` также необязательно
```c
if a == 1 { /* Не сработает */ } 
if (a == 1) { /* Корректно */ } 
...
Тернарная запись - не поддерживается
```c
test = (a == 1) ? 5 : 7; // Не будет работать
```
